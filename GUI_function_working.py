...
def ephy_plot(path_ephy_dic, graph_dic, mice_nb, protocol_type, condition_type, bandpass_dic, shank_dic, random_choice='Fixed Delay', list_files = [], reward_time=500):
    ...
    if random_choice == 'Fixed Delay':
        ...
        mean = ephy_mean(path_ephy_dic['condition_path'], list_files, bandpass_dic)
...
#whole session average by shank and instantanious phase
    ...
    if graph_dic['ws_average'] or graph_dic['ws_phase']:
        ...
        if graph_dic['ws_average']:
            ephy_plot_average(axes, mean, chanel)
            
            if shank_dic['both']: #if we use both chanel
                if plot_position==0: #if didn't already made the second average plot 
                    chanel +=1 
                    if graph_dic['ws_phase']: #if we also plot phase
                        axes = ax[0,1] #we plot with 2 dimention
                        plot_position +=1
                    else:
                        plot_position +=1
                        axes = ax[plot_position] #when only average plot we have 1 dimention
                        
                    ephy_plot_average(axes, mean, chanel) #axes are set up we can plot the second average
                
                if plot_position>0 and graph_dic['ws_phase']: #if we already made the second average plot and there is phase to plot
                    chanel=0
                    axes = ax[1,0]
                    plot_position =0
        ...
        if graph_dic['ws_phase']:
            ...
            if chanel == 0: #this is simply to have the right name on the variable for ech channel (in order to not calculate them again for the time freq)
                complex_map0, map_times0, freqs0, tfr_sampling_rate0, ampl_map0, power_map0,time_freq_calc0 = ephy_plot_phase(axes, mean, chanel)
            else:
                complex_map1, map_times1, freqs1, tfr_sampling_rate1, ampl_map1, power_map1,time_freq_calc1 = ephy_plot_phase(axes, mean, chanel)
                
            if shank_dic['both']: #if we use both chanel
                if plot_position==0: #if didn't already made the second phase plot 
                    chanel +=1
                    if graph_dic['ws_average']:
                        plot_position+=1
                        axes = ax[1,1]
...
#all session average by shank, time frequency plot (power)            
    ...
    if graph_dic['hm_amplitude'] or graph_dic['hm_power']:
        ...
        if not time_freq_calc0 or time_freq_calc1:
            if shank_dic['both']:
                complex_map0, map_times0, freqs0, tfr_sampling_rate0, ampl_map0, power_map0 = ephy_plot_phase(mean, 0)
                complex_map1, map_times1, freqs1, tfr_sampling_rate1, ampl_map1, power_map1 = ephy_plot_phase(mean, 1)
                ...
                complex_map, map_times, freqs, tfr_sampling_rate, ampl_map, power_map = ephy_plot_phase(mean, chanel)
                delta_freq1 = freqs1[1] - freqs1[0]
                extent1 = (map_times1[0], map_times1[-1], freqs1[0]-delta_freq1/2., freqs1[-1]+delta_freq1/2.)
    ...
    if dic_graph_choice_ephy['Ch_group1_power'] or dic_graph_choice_ephy['Ch_group1_amplitude']:
        Ch_group1_complex_map, Ch_group1_map_times, Ch_group1_freqs, Ch_group1_tfr_sampling_rate, Ch_group1_ampl_map, Ch_group1_power_map = ephy_time_freq(mean, 1)                 
        #This is simply to setup the plotting area (check imshow details to further understand, if interested)
...
def ephy_plot_average(axes, mean, chanel):       
    #print(mean['time'])
...
def ephy_plot_phase(axes, mean, chanel):       
    ...
    return complex_map, map_times, freqs, tfr_sampling_rate, ampl_map, power_map, time_freq_calc
