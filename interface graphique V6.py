...
column11 = [[sg.Text('Chanel 0', justification='center', size=(10, 1))],          
            [sg.Checkbox('Amplitude', default=True,key='Ch_group0_amplitude')],
            [sg.Checkbox('Power', default=True, key='Ch_group0_power')]]      

column21 = [[sg.Text('Chanel 1', justification='center', size=(10, 1))],      
            [sg.Checkbox('Amplitude', default=True,key='Ch_group1_amplitude')],
            [sg.Checkbox('Power', default=True, key='Ch_group1_power')]]

column12 = [[sg.Checkbox('Intensity', default=True, key='Ch_group0_intensity')],
            [sg.Checkbox('Frequency', default=True, key='Ch_group0_frequency')]]      

column22 = [[sg.Checkbox('Intensity', default=True, key='Ch_group1_intensity')],
            [sg.Checkbox('Frequency', default=True, key='Ch_group1_frequency')]]


                    #whole session average
columnA = [[sg.Frame(layout=[[sg.Column(column11),sg.Column(column21)],[sg.Text('________Ridge Line__________')],
                             [sg.Column(column12),sg.Column(column22)] ], title='Time Frequency (by shank)',title_color='red', relief=sg.RELIEF_SUNKEN,pad=(0,10))]]
                #Time frequency
columnB = [[sg.Frame(layout=[[sg.Checkbox('Chanel 0', default=True, key='Ch_group0'), sg.Checkbox('Chanel 1', default=True,key='Ch_group1')]] \
            ,title='Whole session average (by shank)',title_color='red', relief=sg.RELIEF_SUNKEN, pad=(0,10))],
           [sg.Frame(layout=[
    ...
    #behaviour plotting
        if event == '-behaviour plot-':
            ...
            if values['radio_random']:   #If we chose to random delay
                dic_graph_choice_time = {"400":values['400'],"400_400":values['400_400'],"900_400":values['900_400'],\
        ...
        if event == '-electrophy plot-':
            ...
            list_protocol_path_ephy = os.path.normpath(path_dic['list_protocol_path_ephy'][0])
            list_condition_path_ephy = os.path.normpath(path_dic['list_condition_path_ephy'][0])
            dic_graph_choice_ephy = {'Ch_group0':values['Ch_group0'],'Ch_group1': values['Ch_group1'],'Ch_group0_power': values['Ch_group0_power'], 'Ch_group1_power':values['Ch_group1_power'], \
                                    'Ch_group0_amplitude':values['Ch_group0_amplitude'], 'Ch_group1_amplitude':values['Ch_group1_amplitude'], 'Ch_group1_intensity': values['Ch_group1_intensity'],\
                                    'Ch_group1_frequency': values['Ch_group1_frequency'],'Ch_group0_intensity': values['Ch_group0_intensity'], 'Ch_group0_frequency': values['Ch_group0_frequency']}
            ...
            if values['radio_random']:
                ...
                lick_path = os.path.normpath(lick_path[0])
                param_path = os.path.normpath(param_path[0])
                gf.random_ephy(list_protocol_path_ephy, lick_path, param_path, dic_graph_choice_ephy, dic_graph_choice_time, mice_nb, protocol_type, condition_type, bandpass_dic)                        
            
            if values['radio_fixe']:
                gf.ephy_plot(list_condition_path_ephy,dic_graph_choice_ephy, mice_nb, protocol_type, condition_type, bandpass_dic) 
